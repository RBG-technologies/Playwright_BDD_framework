...
export class PracticeFormPage extends PlaywrightActions {
  ...
  async selectGender(gender: string) {
    const label = this.page.locator(`//label[text()='${gender}']`);
    await label.waitFor({ state: "visible" });
    // Use tap for real devices (BrowserStack), click for desktop
  ...
  async selectDOB(year = "2026", month = "2", day = "16") {
    await this.click(this.dobInput);
    await this.select(this.yearSelect, year);
    await this.select(this.monthSelect, month);
    await this.click(`//div[text()='${day}']`);
  }
